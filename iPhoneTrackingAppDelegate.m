...
- (BOOL)tryToLoadLocationDB:(NSString*) locationDBPath forDevice:(NSString*) deviceName
{
	...
	for (int pass=0; pass<1; /*pass<2;*/ pass+=1) {
		...
		while ([results next]) {
			...
			const float timeBucketInterval = dayInSeconds;
			const float timeBucket = (floor(unixTimestamp/timeBucketInterval)*timeBucketInterval);
			...
			const float latitude_index = latitude;   //(floor(latitude*precision)/precision);  
			const float longitude_index = longitude; //(floor(longitude*precision)/precision);
